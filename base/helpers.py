...
def whosdaddy():
    return '%s' % inspect.stack()[2][3]

def getattr_trans(obj, field_name, default=None):
    func = getattr(obj,field_name+'_'+multilingual.languages.get_default_language_code())
    if not func:
        func = getattr(obj,field_name+'_en',default)
    if func is not None:
        return func
    else:
        raise AttributeError(field_name)

def getself_trans(obj,default=None):
    field_name = '%s' % whosdaddy()
    return getattr_trans(obj,field_name,default)